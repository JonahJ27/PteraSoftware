...
class WingKinematicsComparison:
    ...
    def get_section_by_column(self, col_index, real=True):
        ...
        for step in range(self.num_steps):
            ...
            if col_index == len(wing.panels[0]):
                panels_column = wing.panels[:, col_index-1]
                section_points = []
                ...
                for panel in panels_column:
                    ...
                    section_points.append([vertices[1][0], vertices[1][1], vertices[1][2]])
                    section_points.append([vertices[3][0], vertices[3][1], vertices[3][2]])
                ...
                for panel in panels_column:
                    ...
                    section_points.append([vertices[0][0], vertices[0][1], vertices[0][2]])
                    section_points.append([vertices[2][0], vertices[2][1], vertices[2][2]])
    ...
    def plot_section(section_real, section_simulated):
        down_real = []
        up_real = []
        down_simulated = []
        up_simulated = []

        maximum_z = section_real[0][0][2]
        i_peak = 0

        while i_peak + 1 < len(section_real) and \
            maximum_z <= section_real[i_peak + 1][0][2]:
            maximum_z = section_real[i_peak + 1][0][2]
            i_peak += 1

        down_real = section_real[:i_peak+1]
        up_real = section_real[i_peak+1:]

        down_simulated = section_simulated[:i_peak+1]
        up_simulated = section_simulated[i_peak+1:]

        plt.figure(figsize=(8, 6))

        for i in range(len(down_real)):
            x_r = [p[0] for p in down_real[i]]
            z_r = [p[2] for p in down_real[i]]  
            plt.plot(x_r, z_r, color="blue", alpha=0.3)

            x_s = [p[0] for p in down_simulated[i]]
            z_s = [p[2] for p in down_simulated[i]]
            plt.plot(x_s, z_s, color="red", alpha=0.3)

        for i in range(len(up_real)):
            x_r = [p[0] for p in up_real[i]]
            z_r = [p[2] for p in up_real[i]]
            plt.plot(x_r, z_r, color="blue", linestyle="--", alpha=0.3)

            x_s = [p[0] for p in up_simulated[i]]
            z_s = [p[2] for p in up_simulated[i]]
            plt.plot(x_s, z_s, color="red", linestyle="--", alpha=0.3)

        plt.xlabel("x")
        plt.ylabel("z")
        plt.title("Wing Section Comparison (Real vs Simulated)")
        plt.axis("equal")
        plt.grid(True)
        plt.legend(["Real - Down", "Simulated - Down",
                    "Real - Up", "Simulated - Up"])
        plt.show()
    ...
    def plot_theta(self):
        ...
        num_steps = self.num_steps
        delta_time = self.delta_time
        times = np.linspace(0, (num_steps - 1) * delta_time, num_steps)
        alpha = []
        theta = []

        Cross_section_0 = self.get_section_by_column(0, real=True)
        root = np.array([step[0] for step in Cross_section_0])  # (num_steps, 3)
        trackers_1 = [t for t in self.list_trackers if t.endswith("1")]

        for i in range(1, len(trackers_1)):
            Cross_section_n = self.get_section_by_column(i, real=True)

            point_1_n = np.array([step[0] for step in Cross_section_n])   # leading edge
            point_l_n = np.array([step[-1] for step in Cross_section_n])  # trailing edge


            chord_vec = point_l_n - point_1_n  

            theta_n = np.rad2deg(np.arctan2(chord_vec[:, 2], chord_vec[:, 0]))
            theta.append(theta_n)

            leading_edge_vec = point_1_n - root
            print(leading_edge_vec)
            alpha_n = np.rad2deg(np.arctan2(leading_edge_vec[:, 2], leading_edge_vec[:, 1]))
        ...
        for i in range(len(theta)):
             plt.plot(times, theta[i], label=r"$\theta$ for section " + str(i+1))

        plt.plot(times, alpha, label=r"$\alpha$")

        plt.xlabel("Time [s]")
        plt.ylabel("Amplitude")
        plt.title("alpha and theta over time")
        plt.legend()
        plt.grid(True)
