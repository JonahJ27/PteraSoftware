...
class WingCrossSectionMovement:
    ...
    **Contains the following methods:**
    ...
    def generate_wing_cross_sections(
        ...
        # If optitrack is True, then use the optitrack data to generate the movement of the WingCrossSection
        else :
            wing_cross_sections = []
            ...
            for step in range(num_steps):
                ...
                this_n_points_per_side = self.base_wing_cross_section.airfoil.n_points_per_side
                this_resample = self.base_wing_cross_section.airfoil.resample
                this_num_spanwise_panels = self.base_wing_cross_section.num_spanwise_panels
                this_spanwise_spacing = self.base_wing_cross_section.spanwise_spacing
                this_chord = this_airfoil.get_chord_length()
                this_Lp_Wcsp_Lpp = (0,0,0) if column == 'A' else this_airfoil.get_relative_transform()[0]
                these_Angles_Wcsp_to_Wcs_ixyz = (0,0,0) if column == 'A' else this_airfoil.get_relative_transform()[1]
                # Make a new WingCrossSection for this time step.
                this_wing_cross_section = geometry.wing_cross_section.WingCrossSection(
                    airfoil=geometry.airfoil.Airfoil(
                        outline_A_lp=this_shape,
                        resample=this_resample,
                        n_points_per_side=this_n_points_per_side,
                        data = data,
                        column = column,
                    ...
                    num_spanwise_panels=this_num_spanwise_panels,
                    chord=this_chord,
                    Lp_Wcsp_Lpp=this_Lp_Wcsp_Lpp,
                    angles_Wcsp_to_Wcs_ixyz=these_Angles_Wcsp_to_Wcs_ixyz,
